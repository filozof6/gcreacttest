...
const WeatherInfo: FC<any> = () => {
      ...
      content: (
        <>
          <Typography variant="body1" component="span" fontSize="large" color="success">
            {weatherInfo?.temp.max}°C{' '}
            <ArrowUpward color="success" fontSize="small" sx={{ position: 'relative', top: 3 }} />
          </Typography>
          <Typography variant="body1" component="span" fontSize="medium">
            &nbsp;/&nbsp;
          </Typography>
          <Typography variant="body1" component="span" fontSize="small">
            {weatherInfo?.temp.min}°C{' '}
            <ArrowDownward fontSize="small" color="warning" sx={{ position: 'relative', top: 3 }} />
          </Typography>
        </>
      ),
    },
  ...
  return weatherInfoLoaded ? (
    <Grid container spacing={2} data-testid="weatherInfo">
      {cardDataArray.map((cardData: CardData, i) => (
        <Grid key={i} item xs={12} sm={6}>
          <Card
            variant="outlined"
            sx={{
